...
class SASRec(torch.nn.Module):
    def __init__(self, args):
        ...
        self.item_emb = torch.nn.Embedding(self.item_num + 1, args.hidden_units, padding_idx=0).to(args.device)
        self.pos_emb = torch.nn.Embedding(args.maxlen, args.hidden_units).to(args.device)
    ...
    def log2feats(self, log_seqs):
        ...
        log_feats = self.last_layernorm(seqs)  # (U, T, C) -> (U, -1, C) batch * maxlen * emb

        return log_feats

    def forward(self, seqs):  # for training
        log_feats = self.log2feats(seqs)  # batch * maxlen * emb
